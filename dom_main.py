...
def main_page(page: ft.Page):
    ...
    def view_task(task_id, task_text, completed=None, date=None):
        ...
        date_text = ft.Text(value=date or '', color=ft.Colors.GREY, size=12)
        ...
        def delete_task_flet(_):
            main_db.delete_task(task_id)
            load_tasks()

        delete_button = ft.IconButton(icon=ft.Icons.DELETE, icon_color=ft.Colors.RED, on_click=delete_task_flet)

        return ft.Row([checkbox, task_field, date_text, edit_button, save_button, delete_button])
